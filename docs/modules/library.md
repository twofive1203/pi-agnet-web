...
| `lib/snflow-assets.ts` | Bundled SnFlow project asset manifest (SemVer) and embedded file contents for the project extension, skill, agents, and CLI wrapper. Asset `1.8.0` binds new dispatches to reserved run ids and immutable specification snapshots. Asset `1.7.0` added v1 agent-contract dispatch, explicit read-only/check completion-guard metadata, and validated zero-change recovery. |

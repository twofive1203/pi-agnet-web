...
| `lib/snflow-assets.ts` | Bundled SnFlow project asset manifest (SemVer) and embedded file contents for the project extension, skill, agents, and CLI wrapper. Asset `1.6.0` adds the current-task-only `/snflow-spec-review` extension command, whose first current-main-Agent turn produces read-only evidence-backed candidates and requires later explicit user selection before accepted-only Spec/index writes. |

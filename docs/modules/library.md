...
| `lib/agent-event-throttler.ts` | Per-session cumulative `message_update` coalescer. Delivers the first snapshot immediately, retains only the latest pending snapshot, caps delivery to 50 ms intervals, and treats every non-update event as an ordering barrier. `message_end`, `agent_end`, `agent_settled`, and `prompt_settled` also reset the leading-edge clock for the next turn. Wrapper teardown clears its timer. |

...
| `lib/pi-web-config.ts` | Read/write/validate `~/.pi/agent/pi-web.json` for WorkTree, Usage scan scope, Web Terminal settings and env assistant model policy, ChatGPT usage panel, warmup schedule, and backend auto-refresh settings, Grok usage panel toggle, Editor shortcuts, and SnFlow panel preferences such as `includeArchived` (compat key `workflow`; legacy `enabled` ignored). Public `PiWebConfig` exposes only supported sections. Unknown raw root keys such as legacy `trellis` are never consumed and are preserved untouched when supported sections are saved; there is no auto-migration that deletes them. |

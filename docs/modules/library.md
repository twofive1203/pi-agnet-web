...
| `lib/server-access-auth.ts` | Instance access-key domain: scrypt verifier in `server-access.json`, opaque 7-day sessions (token hash only), rotation, atomic owner-only writes, process-local login rate limits. Fail closed on corrupt state. |
| `lib/server-access-policy.ts` | Policy for Proxy/auth routes: public path allowlist, cookie helpers, effective HTTPS / trusted-proxy rules, same-origin checks, and durable `server-access-policy.json` auth-bypass CIDRs (env override supported). |

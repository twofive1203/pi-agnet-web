...
| `lib/workflow-store.ts` | Project-local SnFlow task store under `.pi/snflows/tasks/` (archived tasks in sibling `.pi/snflows/archived/`): canonical cwd resolution, strict parsing with agent-hand-write leniency, atomic writes, revision checks, run records, complete/archive. Parent/child tasks remain flat on disk; one-level children are validated at creation, and parent completion/archive is blocked until child work is finished. Never reads/writes `.trellis/`. |
| `lib/snflow-assets.ts` | Bundled SnFlow project asset manifest (SemVer) and embedded file contents for the project extension, skill, agents, and CLI wrapper. Asset `1.7.0` adds v1 agent-contract dispatch, explicit read-only/check completion-guard metadata, active-run revision validation, and validated zero-change implement recovery. Asset `1.6.0` added the current-task-only `/snflow-spec-review` extension command. |
...
| `lib/workflow-chat-lifecycle.ts` | Current-chat SnFlow native-subagent boundary: prepares selected-task-bound v1-contract dispatch instructions, validates marked `subagent` calls before execution, correlates session/tool-call progress and terminal results, and persists task/run projections without affecting ordinary subagents. Structured child execution/final output/effects outrank wrapper prose; text cancellation is error-only fallback, and observed no-edit implement runs require an explicit validated-no-change result. |

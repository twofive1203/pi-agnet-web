...
| `models/` | GET | List cwd-scoped available models and the project-aware default model, including `supportsImage` capability metadata used by Vision settings and attachment routing. Complete metadata is cached briefly per canonical cwd with in-flight deduplication; `?refresh=1` rebuilds and replaces that cache entry after model/auth configuration changes. |

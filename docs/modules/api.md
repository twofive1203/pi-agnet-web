...
| `web-config/` | GET/PUT | Read/write `~/.pi/agent/pi-web.json` for WorkTree defaults, Usage scan scope, Web Terminal settings, ChatGPT usage panel/warmup schedule settings, Editor implementation/shortcut settings, Grok usage panel toggle, and SnFlow panel preferences such as `includeArchived` (compat key `workflow`; legacy `enabled` ignored). Public config omits Trellis; PUT accepts only supported sections and preserves unknown raw root keys such as legacy `trellis` without consuming them. Removed Trellis HTTP routes are unavailable and no longer read `pi-web.json.trellis`. Also lazily ensures the local ChatGPT warmup scheduler. |

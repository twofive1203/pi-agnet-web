...
| `git/workbench/` | GET | Standalone workbench overview: canonical repository identity, HEAD/dirty/operation state, refs revision, bounded Local/Remote/Tags, remotes, authors, upstream/ahead/behind, and truncation flags. Remote refs are local snapshots; this route never fetches. |
| `git/log/` | GET | Revision-bound, bounded Git log page (default 100, max 200) for All or one allowlisted full ref, with literal message/hash-prefix search, canonical author filter, offset pagination, stale-snapshot rejection, and a per-visible-commit `containedInCurrent` mark computed against the exact current HEAD. |
| `git/operations/` | POST | Strict action-union write API for local/remote checkout, ordinary push, reset, cherry-pick/revert, safe reword/drop, and branch/lightweight-tag creation. Rejects arbitrary args/force fields; revalidates revision/HEAD/ref and capability under a common-dir lock; conflicts auto-abort or return `recoveryRequired`. Local/remote checkout and create-and-checkout share the safe-checkout boundary (non-conflicting local changes may travel; overwrite risk is `CHECKOUT_CONFLICT`). Cherry-pick/revert/reword/drop still require a clean worktree via capabilities. |

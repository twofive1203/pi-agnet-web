...
| `sessions/[id]/` | GET/PATCH/DELETE | Read session detail, rename, delete. Returns `archived: true` for archived sessions. GET reuses a matching live RPC session's parsed manager and falls back to disk for inactive sessions; `sessionStats` aggregates every persisted billed entry so compacted and abandoned-branch history matches the Pi CLI lifetime totals. Optional `sessionPerformance` is a separate accurate aggregate (weighted TPS / TTFT / sample count / provider-model breakdown) from the WebUI sidecar; missing/empty/corrupt sidecars return `null` without failing the detail request. `info.messageCount` / `info.firstMessage` come from the current-branch display transcript (real messages, not the compaction summary). `?view=chat` returns `contextState` plus the latest bounded `transcript` page and omits unbounded model-context messages; the default response still includes compaction-aware `context` for compatibility callers that only need `info`. DELETE awaits live wrapper destroy+flush, then removes artifacts plus both the changed-file and performance sidecars. |
| `sessions/[id]/context/` | GET | Compaction-aware model context for a specific `leafId`; reuses a matching live parsed manager when available. Compatibility endpoint for LLM-shaped context, not the chat transcript. |
| `sessions/[id]/transcript/` | GET | Bounded current-branch display transcript page (`leafId`, `beforeEntryId`, clamped `limit`). Same exact-id, archive, and live-manager reuse as session detail. Invalid leaf/cursor returns 400. |
...
| `cwd/browse/` | GET | List child directories on the WebUI server machine for the project path picker (`?path=`). Empty/missing path returns top-level roots (Home + Windows drives or `/`). Directories only; does not read file contents. Selecting a project still requires `POST /api/cwd/validate`. Local and remote Add Project share this single browse path. |
| `cwd/create/` | POST | Create one direct child folder from the server directory picker. Body `{ parent, name }`; the name must be one safe path segment and creation is non-recursive/exclusive. Returns the canonical created path so the picker can enter it. |
| `cwd/open/` | POST | Open an authorized workspace directory in the host OS file manager (Windows `explorer.exe`, macOS `open`, Linux `xdg-open`). Body `{ cwd }`; rejects paths outside allowed roots. Opens on the WebUI server machine, not the browser client. This is not the Add Project directory chooser. |
...
| `git/status/` | GET | Return detailed Git status (branch, commits, NUL-safe staged/unstaged changes, untracked files, stash) for an authorized cwd. Non-repositories retain the compact-panel `{ status: null }` compatibility response. |
| `git/graph/` | GET | Return bounded commit graph data (commits, parents, refs, local branches) for the compact Git panel; optional `branch` previews one validated local branch. Uses the shared authorized Git executor. |
| `git/commit/` | GET | Return metadata and bounded first-parent/root changed-file stats for a selected commit. Additive `capabilities` project server-authoritative safety reasons for workbench actions. |
| `git/diff/` | GET | Return a bounded read-only unified diff, or binary/too-large/unavailable fallback metadata. Commit mode uses `hash`; working-tree mode uses `scope=staged|unstaged`. Both require authorized `cwd` + `path` and support `oldPath` for renames. |
| `git/stashes/` | GET/POST | GET returns the bounded repository-level stash reflog projection plus independent stash/target revisions; POST creates one named stash with tracked changes and optional untracked files. Both require an authorized cwd; ignored files are never included. |
| `git/stashes/[oid]/` | GET | Lazily return one current stash entry and its bounded, NUL-safe deduplicated tracked/untracked file projection. Full object OID is the stable identity; display ordinals are metadata only. |
| `git/stashes/[oid]/diff/` | GET | Return one bounded tracked or untracked stash file diff with literal path handling and binary/too-large/unavailable fallbacks. |
| `git/stashes/[oid]/actions/` | POST | Strict `apply | pop | drop` action API. Apply/Pop require clean target state and exact stash + target revisions; Drop permits a dirty worktree but revalidates the OID under the common-dir lock. Pop conflicts retain the stash and leave the worktree conflict state for manual resolution. |
| `git/switch/` | POST | Compatibility local-branch switch for the compact panel. Uses common-dir mutation locking, cwd authorization, unmerged/operation-state checks, linked-worktree occupancy validation, and the shared safe-checkout boundary. Non-conflicting staged/unstaged/untracked changes are kept; real overwrite risk, including ignored files, returns 409 `CHECKOUT_CONFLICT`. Dirty worktrees are no longer a blanket refusal. |
| `git/workbench/` | GET | Standalone workbench overview: canonical repository identity, HEAD commit plus full symbolic `headRef`, dirty/operation state, a complete heads/remotes/tags revision digest, bounded Local/Remote/Tags projection, remotes, authors, upstream/ahead/behind, and truncation flags. `revisionComplete=false` means the complete refs digest exceeded the bounded read budget; the first read-only projection remains available while pagination and mutations fail closed. Current HEAD and its upstream are retained when the 5,000-ref UI projection is truncated. Remote refs are local snapshots; this route never fetches. |
| `git/log/` | GET | Revision-bound, bounded Git log page (default 100, max 200) for the heads/remotes/tags All universe or one allowlisted full ref, with literal message/hash-prefix search, canonical author filter, offset pagination, stale-snapshot rejection, and a per-visible-commit `containedInCurrent` mark computed against the exact current HEAD. Custom refs and `refs/stash` are outside All; incomplete snapshots disable load-more. |
| `git/operations/` | POST | Strict action-union write API for local/remote checkout, ordinary push, reset, cherry-pick/revert, safe reword/drop, and branch/lightweight-tag creation. The route enforces the 64 KiB limit against actual UTF-8 body bytes before JSON parsing, rejects arbitrary args/force/unknown fields, and revalidates complete revision, HEAD commit, nullable symbolic HEAD ref, selected ref, and capability under a common-dir lock. Push uses an exact `destination` union: existing-upstream branches require `{ mode: "upstream", expectedUpstreamRef }`; branches without upstream require `{ mode: "explicit", remote, target, setUpstream }`; success may return the actual bounded destination. Stable stale errors include `STALE_HEAD_REF`, `STALE_UPSTREAM`, and `SNAPSHOT_INCOMPLETE`. Conflicts auto-abort or return `recoveryRequired`; checkout keeps non-conflicting local changes while overwrite risk is `CHECKOUT_CONFLICT`. |

...
| File | Purpose |
| --- | --- |
| `components/AppShell.tsx` | Top-level layout, URL state, tab management, Web Terminal bottom-dock toggling, right drawer mode switching between files, optional Trellis tasks, and enabled Yolk Workflow tasks, plus Trellis/Yolk task-to-chat context block insertion. |
...
| `components/ChatInput.tsx` | Input bar, model dropdown, thinking level, tool preset, image upload, file-reference chips, serialized Trellis and Yolk task context blocks, and slash-command autocomplete for extension commands, prompts, skills, and project-local Yolk commands such as `/yolk-new-task`. |
...
| `components/SettingsConfig.tsx` | Settings modal for WorkTree defaults, Usage scan scope, Web Terminal enablement/shell/env settings including Unix and Windows shell choices plus raw/AI env parsing model controls, ChatGPT usage panel and backend auto-refresh settings, Editor implementation/shortcut settings, native Yolk Workflow workspace status/enable/disable/update controls, and optional Trellis panel settings in `pi-web.json`, including Trellis docs guidance, prerequisite/status inspection, install/init, update, proxy controls, Trellis workflow assistant primary/fallback model controls, and Trellis subagent model policy controls. ChatGPT warmup schedule is managed from `ChatGptWarmupDialog` and preserved by settings saves. |
...
| `components/YolkWorkflowPanel.tsx` | Native Yolk Workflow task drawer for enabled workspaces: lists `.yolk/tasks` with unfinished tasks as the default filter and an explicit all-tasks history view, creates minimal tasks with `task.json` and `prd.md`, reads task artifacts, supports focused task selection, and inserts Yolk continuation context blocks into ChatInput. |

...
| `components/AppShell.tsx` | Top-level layout, URL state, tab management, language toggle, Web Terminal bottom-dock toggling, right drawer mode switching between files, always-available SnFlow tasks, and optional Trellis tasks, Trellis-task-to-chat context block insertion, body-portaled top-bar auxiliary panels that remain visible outside the mobile horizontal scroller, and a shared right-panel width (SnFlow / file preview / Trellis) persisted under `localStorage` key `pi-web-right-panel-width-v1`. Wide desktop (≥920px) docks the panel inline with pointer/keyboard resize and viewport clamping; narrow desktop (641–919px) uses an overlay drawer so chat is not compressed and hides the resize handle; ≤640px keeps the existing full-width mobile drawer. |
| `components/SessionSidebar.tsx` | Session tree sidebar, workspace/WorkTree picker actions grouped by main workspace, archive/unarchive actions, archived section, multi-select batch archive, and an integrated Explorer that starts collapsed and mounts its file tree only when expanded. Desktop Explorer height is pointer/keyboard-resizable between the session list and file tree, clamped to keep both regions usable, and persisted under `localStorage` key `pi-web-explorer-height-v1` without clearing on collapse. At ≤640px the resize handle stays hidden and the expanded Explorer continues to share remaining height equally with the session list. |

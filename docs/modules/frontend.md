...
| `components/YolkWorkflowPanel.tsx` | Native Yolk Workflow task drawer for enabled workspaces: lists `.yolk/tasks`, creates minimal tasks with `task.json` and `prd.md`, reads task artifacts, supports focused task selection, and inserts Yolk continuation context blocks into ChatInput. |

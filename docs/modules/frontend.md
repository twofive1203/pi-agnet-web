...
| `components/ChatWindow.tsx` | Memoized message list, SSE streaming, fork/navigate logic. Parent-shell subagent progress updates do not rerender the chat when its session props and callbacks are unchanged. Shows archived banner and disables input for archived sessions. Hosts Pi extension status chips, generic widget stacks around the composer, the floating todo panel, and body-portaled dialog/toast hosts for extension UI requests. |
...
| `hooks/useAgentSession.ts` | Central chat/session hook: data loading, SSE, streaming state, commands, tools, model metadata, thinking levels, and bounded subagent summaries. Ordinary server progress no longer carries output; compatibility/final/persisted output is capped to an 8k preview while full bounded detail loads on expansion. Summary state stays outside React chat state and flushes at 300 ms, with terminal/attention updates immediate. The AppShell callback writes an external store rather than top-level React state. Optional browser metrics use `NEXT_PUBLIC_PI_WEB_SUBAGENT_OBSERVABILITY=1`. The hook also owns extension UI state and response bridging. |

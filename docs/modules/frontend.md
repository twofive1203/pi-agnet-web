...
| `components/WorkflowPanel.tsx` | WebUI-owned Workflow drawer: create/select tasks under `.pi/workflows/tasks/` (including one-click create-from-current-chat), edit requirements/design/plan Markdown, mark ready, dispatch implement/check through native pi-subagents, poll run status, record commit metadata, complete, and archive. Auto-selects the cwd current task pointer. Strictly separate from Trellis. |
| `components/WorkflowSessionWidget.tsx` | Floating current-Workflow-task capsule (plan/execute/finish), opens the Workflow drawer focused on that task. |

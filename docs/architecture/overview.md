...
- Some relay providers return empty assistant messages with `stopReason: "stop"` + `rawStopReason: "completed"` and zero usage after tool results. Interactive sessions load the hidden inline extension from `lib/empty-completed-retry.ts`, which rewrites only whitelisted `provider/model` pairs into a retryable assistant error so Pi's existing auto-retry path can continue without injecting a user "继续" message. Default whitelist is `cun1/kimi-k3`; override with `PI_WEB_EMPTY_COMPLETED_RETRY_MODELS=provider/model,...`. Final failures surface as a persistent chat error card plus assistant `errorMessage` rendering.

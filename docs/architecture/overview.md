...
- Web sessions bind Pi extensions in RPC mode through `lib/extension-web-ui.ts`. Simple extension UI requests (`notify`, `confirm`, `select`, `input`, `editor`, status/widget/title/editor text updates) are forwarded through the existing SSE stream; browser responses are returned with `extension_ui_response`. The chat UI renders `setStatus` chips and `setWidget` stacks around the composer, blocking dialogs via `ExtensionDialogHost`, and non-blocking `notify` toasts via `ExtensionToastHost` (no `window.alert`/`confirm`/`prompt`). TUI-only APIs (`custom` components, powerbar chrome, etc.) still degrade with `extension_error` diagnostics instead of blocking silently.

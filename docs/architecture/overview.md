...
- **Instance access gate:** root `proxy.ts` enforces optional global access-key authentication when `PI_WEB_SERVER_MODE=1` (set by `--server`, non-loopback bind, or env). It rejects cross-origin state-changing requests, requires effective HTTPS by default, and returns `401` only after those security gates pass. Opaque HttpOnly sessions live in `server-access.json` (verifier + token hashes only). This gate is independent of and does **not** relax Automation, native picker, or browser-bridge loopback-only policies. The only deliberate cookie/same-origin exemption is Chrome-extension installation pairing (`/api/browser/pair` `exchange`/`connect_token` and `/api/browser/unpair`) so the unpacked extension can talk to `http://127.0.0.1`; those handlers still require a proven loopback TCP peer.
...
- Idle teardown is **settlement-based**: the 10-minute destroy timer starts only after the wrapper is genuinely settled (`agent_settled` / classified `prompt_error` / non-agent `prompt_settled`) **and** there are zero active tools, Subagents, and blocking extension UI requests. Silent long-running tools must not be destroyed merely because no further events arrive or because browser SSE listeners drop to zero. Task-observer reads never count as chat SSE listeners and never extend settled retention.
- Ordinary Agent prompt activities are projected server-side by `lib/task-observer-agent.ts` on the raw event boundary (before SSE throttling): prompt epoch, retries/steer inside the current activity, safe tool/Subagent counters, blocking `extension_ui` → `needs_input` without dialog content, and classified reason codes without raw provider text. `AgentSessionWrapper.getTaskObservation()` exposes the bounded activity for the desktop-observer hub.
- Desktop pet observation is a separate local-only surface under `/api/desktop-observer/**` (`lib/task-observer-hub.ts` + `lib/desktop-observer-access.ts`). One hub builds a bounded multi-source snapshot (Agent, SnFlow with host dedupe, Automation, Quick Command), pushes full snapshots over a single SSE with 500 ms progress coalesce and immediate attention/terminal flush, and never becomes a chat SSE listener or widens `/api/health`. Access requires direct IPv4 loopback Host `127.0.0.1`, a short-lived hashed observer token, and (in server mode) an access key at session mint. Protocol probe may advertise additive `capabilities` such as `quick_session` without bumping the observer protocol version. Public health remains task-metadata-free.
- User-initiated first-message starts use a separate `/api/desktop-control/**` namespace (`lib/desktop-control-access.ts`, `lib/desktop-project-catalog.ts`, `lib/desktop-quick-session.ts`, `lib/desktop-quick-session-models.ts`) with an independent scoped token. Control responses never echo cwd/Prompt; browser and desktop share `lib/new-agent-session.ts`. Observer tokens cannot authorize control routes and the reverse is also true.
- The Windows desktop pet (`desktop/`) is an attach-only Electron companion: main owns networking/observer+control tokens/read-notification LRU/tray/window; preload exposes a narrow `snailPet` bridge; renderer shows pet + project-grouped Activity tray and an optional in-tray first-message composer. Deep links are relative allowlisted paths re-validated before `shell.openExternal`. The pet never spawns/stops/`signal`s `spi`, never stores service PIDs, and quitting the pet does not affect running tasks. Packaging is pet-only (`forge.config.ts` + `DESKTOP_PACKAGE_CONTRACT`), published separately from npm `spi`; uninstall must leave `~/.pi/agent` untouched. Contract smoke: `npm run test:desktop-package`. Manual Windows installer/signing matrix: `docs/operations/desktop-pet-validation.md`.
...
- Web sessions bind Pi extensions in RPC mode through `lib/extension-web-ui.ts`. Simple extension UI requests (`notify`, `confirm`, `select`, `input`, `editor`, status/widget/title/editor text updates) are forwarded through the existing SSE stream; browser responses are returned with `extension_ui_response`. The chat UI renders `setStatus` chips and generic `setWidget` stacks around the composer; the standard `todo-list` widget is projected into a floating task panel with progress and parsed task rows. Blocking dialogs use `ExtensionDialogHost`, and non-blocking `notify` uses `ExtensionToastHost` (no `window.alert`/`confirm`/`prompt`). For `setWidget`, string arrays are forwarded directly; TUI component factories (e.g. `manage_todo_list`) are invoked with a passthrough theme and materialized via `render(width)` into plain text lines. Factories that cannot be materialized (full TUI/theme deps) are dropped silently, matching official RPC mode. pi-subagents TUI HUDs that overlap the top-bar SubagentPanel (`subagent-fleet-status`, `subagent-async`) and powerbar chrome (`powerbar`, `powerbar:*`) are intentionally suppressed in the Web bridge (no materialize/SSE). Other TUI-only APIs (`setFooter`, `setHeader`, `setEditorComponent`, working-message chrome, `custom`, autocomplete providers) are silent no-ops so extensions such as pi-powerbar / pi-raw-paste do not spam `extension_error` on every session bind.
...
- Prompt-level running state settles on `agent_settled`, not bare `agent_end`. `agent_end` may be followed by auto-retry backoff, auto-compaction recovery, or queued follow-ups; the WebUI keeps `agentRunning` true across that gap and only clears on `agent_settled` (or `prompt_error` / non-agent `prompt_settled`). Follow-up UI consumes Pi's full `queue_update.followUp` snapshot and the live `get_state.followUpMessages` snapshot: pending items render in a separate Composer-adjacent queue, then move into the transcript only when the matching queued user `message_start` begins.

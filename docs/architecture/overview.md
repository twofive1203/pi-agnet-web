...
- Native Yolk Workflow state is independent from Trellis: setup/status/task APIs read and write only `.yolk/` plus yolk-prefixed project-local `.pi/` resources, track managed template hashes in `.yolk/manifest.json`, and report conflicts instead of overwriting modified or unmanaged files. The first version does not auto-reload already-open AgentSessions after provisioning; new sessions pick up project-local resources through normal Pi discovery.

...
| Select Codex v2 | Running/Needs input/Ready/Blocked map to running/waiting/waving/failed; Snail labels/glyphs unchanged. Ready does not loop `jumping`. Idle/running/retrying click plays one-shot `jumping` plus outer poke/flail. Running `thinking` plays `review`. Bitmap contain keeps 192:208 at small/medium/large. | Automated profile mapping, clip reachability, and CSS cascade/size smoke; visual playback 未执行 |

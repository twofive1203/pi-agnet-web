...
- **Incompatible / protocol mismatch / legacy server mode:** the port owner is not a compatible Snail Pi observer. Fix/update the service or change the pet port; the pet will not kill or replace the process. Current server mode is attachable only through direct `127.0.0.1` and requires its access key in desktop pet settings; missing/invalid keys show a dedicated diagnostic.

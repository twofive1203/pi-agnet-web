...
| `npm run test:git-stash` | Inspector stash list/detail/diff/create/apply/pop/drop safety smoke suite with temp repos and linked worktrees. |
...
| Workspace files and Git context | `app/api/files/**`, `app/file/page.tsx`, `app/git/page.tsx`, `components/StandaloneFileViewer.tsx`, `components/git-workbench/`, `components/GitStash*.tsx`, `app/api/git/**`, `lib/file-paths.ts`, `lib/file-viewer-url.ts`, `lib/git-executor.ts`, `lib/git-branch-switch.ts`, `lib/git-repository-status.ts`, `lib/git-stash*.ts`, `lib/git-workbench*.ts`, `lib/git-worktree.ts`, `lib/workspace-title.ts` | `docs/modules/api.md`, `docs/modules/frontend.md`, `docs/modules/library.md` |

...
const QUERY_CREDENTIAL_RE = /([?&](?:token|access_token|refresh_token|id_token|auth|authorization|api_key|apikey|key|secret|password|passwd|session|sid|code|client_secret|signature|sig)=)[^&\s"'<>]*/gi;
const PLAIN_TOKEN_ASSIGN_RE = /\b((?:access[_-]?token|refresh[_-]?token|id[_-]?token|api[_-]?key|apikey|client[_-]?secret|session[_-]?token|auth[_-]?token|token|secret|password|passwd)\s*[=:]\s*)("?[^\s"'&,;<>]+"?)/gi;
...
export function isSensitiveControl(meta) {
    ...
    if (PAYMENT_ROLE_RE.test(meta.role ?? "") || PAYMENT_ROLE_RE.test(meta.label ?? ""))
        return true;
    return false;
}
...
export function sanitizeConsoleText(text, maxChars = 2_000) {
...
export function sanitizeConsoleValue(value, maxChars = 2_000, depth = 0) {
    if (value == null)
        return value;
    if (typeof value === "string")
        return sanitizeConsoleText(value, maxChars);
 ...
 */
export function summarizeAuditParams(params) {
    if (!params)
        return undefined;
    const out = {};
        ...
        if (typeof value === "string") {
            if (key.toLowerCase().includes("url")) {
                ...
                if (typeof out[key] === "string")
                    out[key] = sanitizeConsoleText(out[key], 80);
            }
        }

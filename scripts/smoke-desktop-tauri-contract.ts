...
assert.equal(petWindow.width, 197);
assert.equal(petWindow.height, 235);
assert.equal(petWindow.minWidth, 164);
assert.equal(petWindow.minHeight, 196);
...
assert.doesNotMatch(appState, /tray_anchor:\s*Some\("top-left"\)/);
assert.match(windowController, /pet_layout_spec/);
assert.match(windowController, /transition_window_layout/);

...
export const settingsZh = {
  ...
  workflowDescription: "SnFlow 是 Snail Pi Web 自有开发工作流，任务保存在 .pi/snflows/tasks/（归档任务在同级 archived/），通过 native pi-subagents 的 worker/reviewer 执行实现与检查。与 Trellis 完全独立。",
  enableWorkflow: "启用 SnFlow 面板",
  enableWorkflowHint: "开启后右侧会出现 SF 按钮；API 也会检查该开关。SnFlow 按项目启用：每个项目还需在 SnFlow 面板中初始化（创建 .pi/snflows/）后，对话才会注入 SnFlow 引导。",
  workflowIncludeArchived: "默认包含已归档任务",
  workflowIncludeArchivedHint: "仅影响面板列表默认筛选。",
  workflowNativeModelsHint: "SnFlow 子代理模型仍使用 Settings → Agents 中的 native pi-subagents 配置（settings.json → subagents），不会读取 trellis.subagents。",
...
export const settingsEn = {
  ...
  workflowDescription: "SnFlow is the Snail Pi Web-owned development flow. Tasks live under .pi/snflows/tasks/ (archived tasks in the sibling archived/) and dispatch implement/check through native pi-subagents worker/reviewer. Fully separate from Trellis.",
  enableWorkflow: "Enable SnFlow panel",
  enableWorkflowHint: "Shows the SF button on the right; APIs also require this setting. SnFlow is enabled per project: each project must be initialized from the SnFlow panel (creates .pi/snflows/) before chat guidance is injected.",
  ...
  workflowNativeModelsHint: "SnFlow agent models still come from Settings → Agents (settings.json → subagents). SnFlow never reads trellis.subagents.",

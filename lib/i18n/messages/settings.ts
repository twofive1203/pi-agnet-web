...
export const settingsZh = {
  ...
  workflowDescription: "SnFlow 是 Snail Pi Web 自有开发工作流，任务保存在 .pi/snflows/tasks/（归档任务在同级 archived/），通过 native pi-subagents 的 worker/reviewer 执行实现与检查。与 Trellis 完全独立。右侧 SF 面板始终可用；是否走 SnFlow 只取决于当前项目有没有初始化（.pi/snflows/）。初始化会写入项目级 pi 扩展、skill、agent 与版本文件。",
